  ...
  void print_array_graph(int* A, int p, int r, string s,int pivot){
    ...
    output_graph << s << p << "_" << r << " [label=<"<< endl<< "<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" ";
  ...
  int main(int argc, char **argv) {
    ...
    for (n = max_dim/ndiv;n<=max_dim;n+=max_dim/ndiv){
      ...
      if (ndiv>1)
        printf("%d,%d,%d,%.1f,%d,%.1f,%d,%.1f,%d,%.1f,%.1f\n",n,ntests,swap_min,(0.0+swap_avg)/ntests,swap_max,0.0+n*n,cmp_min ,(0.0+cmp_avg)/ntests ,cmp_max ,0.0+n*n,0.0+n*log(n)/log(2));
    }
